...
export const Login: React.FC = () => {
  const { login, isRealFirebase } = useAuth();
        ...
        {isRealFirebase ? (
          ...
          <div className="glass-panel" style={{
            ...
            <h4 style={{ fontSize: '1rem', fontWeight: 700, marginBottom: '4px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              Developer Sandbox Sandbox Mode
            </h4>
            <p style={{ fontSize: '0.825rem', color: 'hsl(var(--text-secondary))', marginBottom: '20px' }}>
              Firebase config was not found. Select a simulated profile below to evaluate the system's role-based behavior.
            </p>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <button 
                onClick={() => handleMockLogin('admin-1')}
                className="btn btn-secondary"
                style={{ justifyContent: 'space-between', padding: '12px 16px', fontSize: '0.9rem' }}
              >
                <span style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span className="badge badge-admin">Admin</span>
                  <strong>Eleanor Vance</strong> (Platform Admin)
                </span>
                <ArrowRight size={16} />
              </button>

              <button 
                onClick={() => handleMockLogin('coach-1')}
                className="btn btn-secondary"
                style={{ justifyContent: 'space-between', padding: '12px 16px', fontSize: '0.9rem' }}
              >
                <span style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span className="badge badge-user">Coach</span>
                  <strong>Marcus Brody</strong> (Approved Coach)
                </span>
                <ArrowRight size={16} />
              </button>

              <button 
                onClick={() => handleMockLogin('pending-1')}
                className="btn btn-secondary"
                style={{ justifyContent: 'space-between', padding: '12px 16px', fontSize: '0.9rem' }}
              >
                <span style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <span className="badge badge-pending">Pending</span>
                  <strong>Aria Sterling</strong> (New Coach / Pending Review)
                </span>
                <ArrowRight size={16} />
              </button>

              <button 
                onClick={() => handleMockLogin(`new-user-${Date.now()}`)}
                className="btn btn-secondary"
                style={{ justifyContent: 'space-between', padding: '12px 16px', fontSize: '0.9rem', borderStyle: 'dashed' }}
              >
                <span style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'hsl(var(--text-secondary))' }}>
                  <UserPlus size={16} />
                  <em>Register a New Unapproved User</em>
                </span>
                <ArrowRight size={16} />
              </button>
            </div>
